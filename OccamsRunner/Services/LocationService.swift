...
class LocationService: NSObject, ObservableObject {
  ...
  var saveBlockerDescription: String {
    ...
    if q.matchedSampleRatio < 0.65 {
      lines.append(
        "• Match \(Int(q.matchedSampleRatio * 100))% — needs ≥65% (GPS↔AR correlation too low)")
    }
    if q.averageFeaturePoints < 75 {
      lines.append(
        "• Features \(Int(q.averageFeaturePoints)) — needs ≥75 (scan more textured surfaces)")
    }
    if q.averageTrackingScore < 0.65 {
      lines.append(
        "• Tracking \(Int(q.averageTrackingScore * 100))% — needs ≥65% (move more slowly)")
    }
  ...
  func stopRecording() {
    ...
    worldMapTimer = nil
    logDebug(
      "Recording stopped by user. geoSamples=\(geoDraftSamples.count), matchedLocal=\(localDraftBySampleId.count)"
    )
  ...
  func distanceToItem(_ item: QuestItem, route: RecordedRoute) -> Double? {
    guard let current = currentLocation,
      let itemLocation = item.resolvedGeoLocation(on: route)
    else { return nil }
  ...
  private func nearestLocalFrame(to timestamp: Date, tolerance: TimeInterval = 0.35)
    -> LocalFrameSample?
  {
  ...
  private func recordLocationSample(
    ...
    if geoDraftSamples.count <= 5 || geoDraftSamples.count % 10 == 0 {
      ...
      logDebug(
        "\(context) #\(geoDraftSamples.count) " + "hAcc=\(Int(location.horizontalAccuracy))m "
